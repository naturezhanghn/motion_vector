...
size = 8
thre = 5
thre_judge_shake = 0.2 * ( size * np.sqrt(2) )
...
def detectMotionVector(pre, cur) : 
    ...
    for x in range(k, w - k - 1, 2 * k + 1):
        ...
        for y in range(k, h - k - 1, 2 * k + 1):
            ...
            vecs.append(vec)
            vecs1.append(vec1)
            if vec1 < thre_judge_shake:  #Ignore high speed moving targets
                cnt = cnt + 1
                sum = sum + vec1
    ...
    print(end - start)
    return vecs , vecs1,  sum/cnt  #sum/cnt is used to determine if it is a shake

num = 4 #represent a4.mp4
data_path = "/workspace/mnt/storage/zhangziran/zhangzr/motion_vector/data/dataset_480_270/"
save_path = "/workspace/mnt/storage/zhangziran/zhangzr/motion_vector/test/test_motion_vector/testv1_1/"
...
def main():    
    ...
    for i in range(30,1000,30): 
        ...
        for j in range(1,27,2):
                ...
                frame0 = cv2.imread(data_path+"a"+str(num)+"/a"+str(num)+"_"+str(-j+i+1)+".png")
                frame1 = cv2.imread(data_path+"a"+str(num)+"/a"+str(num)+"_"+str(-j+i)+".png")
    ...
    f=open(os.path.join(save_path,"average.txt"),"w")
    for line in average_motion:
        f.write(str(line)+'\n')
        #f.write(str(average_motion))
    f.close()

    f=open(os.path.join(save_path,"abs_vector.txt"),"w")
